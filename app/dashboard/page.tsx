...
const PROGRAMS: Programs = {
  ...
  "Хімія": {
    "7 клас": {
      id: "chemistry-7",
      classes: [7],
      description: "Початкові хімічні поняття (1.5 год/тиждень, 51 год)",
      lessonsPerWeek: [1, 2],
      hasVariant: false,
    },
    "8 клас": {
      id: "chemistry-8",
      classes: [8],
      description: "Будова атома, хімічний зв'язок, класи сполук (2 год/тиждень, 68 год)",
      lessonsPerWeek: 2,
      hasVariant: false,
    },
    "9 клас": {
      id: "chemistry-9",
      classes: [9],
      description: "Розчини, хімічні реакції, органічні сполуки (2 год/тиждень, 68 год)",
      lessonsPerWeek: 2,
      hasVariant: false,
    },
                ...
                <div className="relative">
                  ...
                  >
                    {Object.keys(PROGRAMS[formData.subject] || {}).map((prog) => (
                      <option key={prog} value={prog} className="bg-slate-800">{prog}</option>
                    ))}
                  </select>
                  <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none">
                    <svg className="w-5 h-5 text-cyan-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="relative">
                  ...
                  >
                    {currentProgram?.classes.map((cls: number) => (
                      <option key={cls} value={cls.toString()} className="bg-slate-800">{cls} клас</option>
                    ))}
                  </select>
                  <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none">
                    <svg className="w-5 h-5 text-cyan-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {(() => {
                  ...
                  return (
                    <div className={`mb-4 rounded-xl p-4 border ${
                      selectedCount === 0 ? 'bg-cyan-500/10 border-cyan-500/30'
                      ...
                      <p className="text-sm flex items-start gap-2">
                        ...
                        <span className={selectedCount === 0 ? 'text-cyan-300' : isCorrect ? 'text-green-300' : isOver ? 'text-red-300' : 'text-amber-300'}>
                          {selectedCount === 0
                            ...
                            : isCorrect ? <>Чудово! Вибрано {selectedCount} {selectedCount === 1 ? 'день' : 'дні'} — відповідає програмі ({hint})</>
                            : isOver ? <>Забагато днів! Вибрано {selectedCount}, а програма передбачає {hint}</>
                      ...
                      <label key={day.id} className="relative group cursor-pointer">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            const newDays = e.target.checked ? [...days, day.id] : days.filter(d => d !== day.id);
                            setFormData({ ...formData, weekdays: newDays.join(',') });
                          }}
                          className="peer sr-only"
                        />
                        <div className={`relative overflow-hidden rounded-xl p-4 border-2 transition-all duration-300 ${
                          isSelected ? `bg-gradient-to-br ${day.gradient} border-transparent shadow-lg scale-105`
                          ...
                          <div className="relative flex items-center justify-between">
                            ...
                            <div className={`w-7 h-7 rounded-lg flex items-center justify-center transition-all ${
                              isSelected ? 'bg-white/20 backdrop-blur-sm' : 'bg-slate-600/50 border-2 border-slate-500'
                            }`}>
                              {isSelected && (
